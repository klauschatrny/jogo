...
func test_create_new_usa_balance() -> void:
	...
	assert_eq(p.stats.max_hp, 120)   # BASE_PHP
	assert_eq(p.stats.current_hp, 120)
...
func test_take_damage_negativo_e_clampado() -> void:
	var p := _player()
	p.take_damage(-10)
	assert_eq(p.stats.current_hp, 120, "dano negativo não deve curar")

func test_heal_respeita_max() -> void:
	var p := _player()
	p.take_damage(50)   # 70
	p.heal(100)
	assert_eq(p.stats.current_hp, 120, "cura não passa do max_hp")
